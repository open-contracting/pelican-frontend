...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    removeResourceCheckExpandedNode(state, section) {
      state.resourceCheckExpandedNodes =
        state.resourceCheckExpandedNodes.filter((v) => !v.startsWith(section));
    },
  },
  actions: {
    loadSettings({ commit }) {
    ...
    loadResourceLevelStats({ commit, state }) {
      return new Promise((resolve) => {
        ...
        var formatted = CONFIG.apiEndpoints.resourceLevelReport.replace(
          /{id}/g,
          state.dataset.id,
        );
        axios
    ...
    loadDatasetLevelStats({ commit, state }) {
      return new Promise((resolve) => {
        ...
        var formatted = CONFIG.apiEndpoints.datasetLevelReport.replace(
          /{id}/g,
          state.dataset.id,
        );
        axios
    ...
    loadFieldLevelStats({ commit, state }) {
      return new Promise((resolve) => {
        ...
        var formatted = CONFIG.apiEndpoints.fieldLevelReport.replace(
          /{id}/g,
          state.dataset.id,
        );
        axios
    ...
    loadTimeVarianceLevelStats({ commit, state }) {
      return new Promise((resolve) => {
        ...
        var formatted = CONFIG.apiEndpoints.timeVarianceLevelReport.replace(
          /{id}/g,
          state.dataset.id,
        );
        axios
        ...
        if (getters.fieldCheckSearch) {
          ...
          nodes = nodes.filter((n) => {
            ...
            return matched.some((m) => {
              return (
                m.startsWith(n + ".") && isPathSearched(m.substr(n.length))
              );
            });
          });
