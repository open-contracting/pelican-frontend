  ...
  methods: {
    ...
    sortByCoverage: function (checks, asc = true) {
      this._sort(
        checks,
        (a, b) => {
          var comparison = this.compareNumbers(
            a.coverageOkShare,
            b.coverageOkShare,
          );
          if (comparison == 0) {
            comparison = this.compareNumbers(
              a.coverage.total_count,
              b.coverage.total_count,
            );
          }
          if (comparison == 0) {
    ...
    sortByQuality: function (checks, asc = true) {
      this._sort(
        ...
        (a, b) => {
          ...
          if (comparison == 0) {
            comparison = this.compareNumbers(
              a.quality.total_count,
              b.quality.total_count,
            );
          }
          if (comparison == 0) {
            comparison = a.path.localeCompare(b.path);
