...
The API is used by the Vue frontend (see the endpoints in ``frontend/src/config.ts``) and for managing datasets in Pelican backend (see the `deploy documentation <https://ocdsdeploy.readthedocs.io/en/latest/use/pelican.html>`__).
