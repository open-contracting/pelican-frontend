...
.. note::
   ...
   The keys of checks, sections and stages are referenced dynamically, like ``$t("datasetLevel." + check.name + ".name")``, so a search for a full key finds no uses.
...
#. Add new keys to ``fieldDetail.coverage`` and/or ``fieldDetail.quality`` in ``frontend/src/messages/en.json``
...
#. Add new keys to ``resourceLevel.coherent``, ``resourceLevel.consistent`` and/or ``resourceLevel.reference`` in ``frontend/src/messages/en.json``
...
#. Add new keys to ``datasetLevel`` in ``frontend/src/messages/en.json``
...
#. Add new keys to ``timeLevel`` in ``frontend/src/messages/en.json``
