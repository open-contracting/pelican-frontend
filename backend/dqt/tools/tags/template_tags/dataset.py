    ...
    def prepare_data(self, data):
        ...
        if check.result is not None:
            if check_type == "donut":
                ...
                new_data["passedExamples"] = [example["original_process"]["ocid"] if "original_process" in example else example["ocid"] for example in check.meta["passed_examples"]]
                new_data["failedExamples"] = [example["original_process"]["ocid"] if "original_process" in example else example["ocid"] for example in check.meta["failed_examples"]]
            elif check_type == "biggest_share":
