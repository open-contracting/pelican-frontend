...
class Tag:
    ...
    def set_param_validation(
        self, name, validation, description="The value must be in a valid format.", required=False
    ):
        if self.params_finalized:
            raise ValueError(
                "The parameters for this tag have been already finalized. New parameter validations cannot be set."
            )
    ...
    def set_param(self, name, value):
        if self.params_finalized:
            raise ValueError(
                "The parameters for this tag have been already finalized. New parameter values cannot be set."
            )
...
class TemplateTag(Tag):
    ...
    FULL_TAG_LOCATION_XPATH = (
        './/*[.//text()[contains(., "{full_tag}")] and not(./*//text()[contains(., "{full_tag}")])]'
    )
    DEFAULT_STYLES = (
        "Standard",
        "Heading",
    ...
    def get_template_content(self, template):
        return template.xpath(
            "//office:text", namespaces={"office": "urn:oasis:names:tc:opendocument:xmlns:office:1.0"}
        )[0]
    ...
    def get_template_fonts(self, template):
        return template.xpath(
            "//office:font-face-decls", namespaces={"office": "urn:oasis:names:tc:opendocument:xmlns:office:1.0"}
        )[0]
            ...
            for sub_style in sub_template_styles:
                ...
                for attrib_name in list(sub_style.attrib.keys()):
                    if (
                        ...
                        and sub_style.get(attrib_name) not in TemplateTag.DEFAULT_STYLES
                    ):
    ...
    def merge_template(self, sub_template, full_tag):
        ...
        for child in sub_template_content.iter():
            for attrib_name in child.attrib:
                if (
                    re.match(r"\{[^}]*\}style-name", attrib_name)
                    and child.get(attrib_name) not in TemplateTag.DEFAULT_STYLES
                ):
